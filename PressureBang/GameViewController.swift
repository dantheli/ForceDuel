...
class GameViewController: UIViewController {
    ...
    @IBOutlet weak var player2ForceLabel: UILabel!
    @IBOutlet weak var player1ForceLabel: UILabel!
    ...
    func updateForce(notification: NSNotification) {
        let userInfo = notification.userInfo as! Dictionary<String, Int>
        let player = userInfo["player"]!
        if let force = userInfo["force"] {
            if player == 1 {
                player1ForceLabel.text = "\(force)"
            } else if player == 2 {
                player2ForceLabel.text = "\(force)"
            }
        }
    }
